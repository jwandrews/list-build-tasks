    ...
    switch (taskRunner) {
      case "grunt": {
        ...
        try {
          ...
          results = grep("grunt.registerTask", pathToTaskFile);
        } catch (e) {
      ...
      case "gulp": {
        let results, requireDirFound;

        try {
          // grep the taskfile for tasks
          results = grep("gulp.task", pathToTaskFile);
          // grep the taskfile for directory require statement
          requireDirFound = grep("requireDir", pathToTaskFile);
        } catch (e) {
          reject({ message: e.message });
          break;
        }
        ...
        if (requireDirFound.length >= 1) {
          ...
          let gulpDirectory = path.join(
            pathToTaskFile,
            "..",
            stringIncludesDirectory.join().split("'")[1]
          );
